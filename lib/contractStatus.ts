 ...
 *
 * Contract lifecycle:
 *   sent → signed → confirmed (escrowed) → paid (talent wallet credited)
 *
 * "paid" = agency released payment → talent's BrisaHub wallet was credited.
 * This is the end of the CONTRACT lifecycle. Whether the talent subsequently
 * withdrew to PIX is a SEPARATE event in wallet_transactions — it has nothing
 * to do with the contract status and must NOT be shown as contract state.
 */

export type ContractPaymentStatus =
  | "paid_to_wallet"  // paid — talent wallet credited — contract is complete
  | "escrow"          // confirmed — held in escrow pending agency action
  | "signed"          // signed by talent — awaiting agency deposit/confirmation
  | "pending"         // sent — awaiting talent signature
  | "cancelled"
  | "rejected";
...
export function contractStatusLabel(status: ContractPaymentStatus): string {
  const labels: Record<ContractPaymentStatus, string> = {
...
export function contractStatusTone(status: ContractPaymentStatus): string {
  const tones: Record<ContractPaymentStatus, string> = {
...
/**
 * Read stored financial amounts for a contract.
 * Never recalculate commission or net from plan rates for historical contracts.
 * Stored values are set at contract creation time and reflect the plan in
 * effect at that moment — they must not be overwritten by current plan rates.
 */

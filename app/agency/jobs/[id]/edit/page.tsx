...
export default async function EditJobPage({ params }: Props) {
  ...
  const { data: job } = await supabase
    .from("jobs")
    .select("id, title, description, category, budget, deadline, job_date, job_time, status, location, gender, age_min, age_max, number_of_talents_required, application_requirements, agency_id, workspace_id")
    .eq("id", id)
    .single();

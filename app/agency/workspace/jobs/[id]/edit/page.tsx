...
export default async function WorkspaceEditJobPage({ params }: Props) {
  ...
  const { data: job } = await supabase
    .from("jobs")
    .select("id, title, description, category, budget, deadline, status, location, gender, age_min, age_max, number_of_talents_required, agency_id, workspace_id, created_by_user_id")
    .eq("id", id)
    .single();
